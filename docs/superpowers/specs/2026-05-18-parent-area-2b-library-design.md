...
- `src/components/parent/library/`:
  - `VettedTab.tsx` — `getLibraryContent('vetted', filters)`. Read-only. Filter bar: subject, grade, TEKS code, RIT band, status (difficulty omitted — not a `LibraryContentRow` column; documented). Per-item: Add to test. Multi-select bulk: Add to test.
  - `MyQuestionsTab.tsx` — `getLibraryContent('my_questions', filters)` (covers `parent_manual` + `parent_ai_assisted`, mapped by the view). Per-item: Archive (`archiveCustom*` by `content_type`), status pill. Top actions: `+ New question` → `/parent/custom-bank/new-question`, `+ New passage` → `/parent/custom-bank/new-passage`. Filters: subject, grade, status. **No Edit action** (deferred — see §9).
  - `AiStudioTab.tsx` — see §8.
  - `LibraryItemCard.tsx` — renders one `LibraryContentRow` (content_type, subject, grade, teks_code/title, rit_band, status pill, created_at), a selection checkbox, and a slot for tab-specific actions. One card component, three callers; it never decides source itself.
...
- Per-card actions: **Publish** (single, with confirm), Archive.
...
- **Preview affordance on Library item cards.** A true content preview requires loading the full question/passage (stem, choices, passage body, SVG) and a modal renderer — `LibraryContentRow` (from `map_v_library_content`) carries metadata only. That content-rendering path overlaps the existing editors / test runner and is out of 2b's lean scope (parallel to the Edit deferral). 2b cards therefore expose **no Preview action**; the card itself is the at-a-glance view (type, subject, grade, TEKS, status, date). Explicit, documented deferral — a real content preview is a later-cycle concern.
