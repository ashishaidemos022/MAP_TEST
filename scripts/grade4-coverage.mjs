// Grade 4 question-bank coverage report.
// Mirrors scripts/grade3-coverage.mjs — same three sections (per-standard
// coverage, misconception-tag rollup, untagged-distractor count).
//
// Run: node --env-file=.env.local scripts/grade4-coverage.mjs
...
if (!SUPABASE_URL || !SUPABASE_KEY) {
  ...
  console.error('Run with: node --env-file=.env.local scripts/grade4-coverage.mjs')
  process.exit(1)
}
...
// Grade 4 RIT centroids per CLAUDE.md §9 norms (G4 sits ~190 mid-year):
// keep the same buckets as G3 — values 181_190 through above_210 are the meaningful zone.
...
console.log('\n' + GREEN('━━━ Grade 4 — Per-standard coverage ━━━'))
...
for (const subject of ['math','reading','language']) {
  ...
  console.log(DIM(`  ${pad('TEKS', 14)} ${pad('Title', 50)} ${lpad('Tot', 4)} ${BANDS.map(b => lpad(b, 9)).join(' ')}`))
  for (const r of rows) {
    const flag = r.total < 6 ? RED : (r.total < 21 ? YEL : GREEN)
    const title = r.teks_title.length > 48 ? r.teks_title.slice(0, 47) + '…' : r.teks_title
    console.log(`  ${pad(r.teks_code, 14)} ${pad(title, 50)} ${flag(lpad(r.total, 4))} ${BANDS.map(b => lpad(r.bands[b], 9)).join(' ')}`)
  }
  console.log()
}
...
console.log(GREEN('━━━ Grade 4 — Misconception tag rollup ━━━'))
...
console.log(GREEN('━━━ Grade 4 — Untagged distractors (must be 0) ━━━'))
